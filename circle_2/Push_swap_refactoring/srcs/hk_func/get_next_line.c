...
static void	gnl_strcpy(char *dest, char *src, size_t size)
{
	size_t	i;
...
static int	clean(int fd, char *buf, char **backup, int n)
{
	if (buf)
	{
		free(buf);
...
static int	gnl_get_one_line(int fd, char **line, char **backup, char *buf)
{
	char	*tmp;
	size_t	len;
